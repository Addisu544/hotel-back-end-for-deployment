...
export const freeToday =(req,res)=>{
  const query = `SELECT r.*,
       CASE
         WHEN COUNT(res.id) = 0 THEN NULL
         WHEN MIN(res.checkinDate) > CURDATE() THEN DATEDIFF(MIN(res.checkinDate), CURDATE())
         ELSE DATEDIFF(MAX(res.checkoutDate), CURDATE()) + 1
       END AS available_today
FROM rooms r
LEFT JOIN reservation res ON r.id = res.roomId AND res.status != 'cancelled'
GROUP BY r.id
