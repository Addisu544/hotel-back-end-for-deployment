...
// export const getFeedback = (req,res) => {
//     const query = `
//         SELECT g.profilePicture, g.firstName, g.lastName, f.feedback, f.created_at 
//         FROM feedback f
//         JOIN guest g ON f.guestId = g.id 
//         ORDER BY f.created_at DESC
//     `;
//     db.query(query, (error, results) => {
//         if (error) {
//             console.log("SQL Error:", error.sqlMessage); // <--- log the exact MySQL error
//             return res.status(500).json({ error: "Internal server error" });
//         }
//         return res.status(200).json({ feedback: results });
//     });
